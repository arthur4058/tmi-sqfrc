"""One-shot 60-second test using validation-frozen recovery/V2 weights."""
...
def formal_test(seed=10086):
    ...
    recovery_logits, recovery_targets = load_recovery_test(seed)
    if not np.array_equal(targets, recovery_targets):
        raise ValueError("V2 and recovery test targets are not aligned")
    ...
    recovery_probability = softmax(
        recovery_logits, calibration["temperatures"]["recovery"]
    )
    weight = calibration["recovery_residual_weight"]
    fusion_probability = (
        (1.0 - weight) * v2_probability
        + weight * recovery_probability
    )

    v2_score = metrics(v2_probability, targets)
    recovery_score = metrics(recovery_probability, targets)
    fusion_score = metrics(fusion_probability, targets)
    result = {
        "protocol": "geolife-representation-recovery-v2-formal-test",
